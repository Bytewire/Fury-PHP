...
class Securimage {
    ...
    */
    ...
    function Securimage()
    {
        ...
        $path = explode("/",$this->basepath);
        array_pop($path);
        $this->resource_path = implode("/",$path);
        ...
        $this->gd_font_file  = $this->resource_path.'/fonts/automatic.gdf';
        $this->use_gd_font   = false;
        $this->gd_font_size  = 24;
        $this->text_x_start  = 15;

        $this->ttf_file      = $this->resource_path . '/fonts/IHATCS.ttf';
        ...
        $this->audio_path   = $this->resource_path . '/audio/';
    ...
    function drawWord()
    {
        ...
        if ($this->use_gd_font == true || !is_readable($this->ttf_file)) {
            if (!is_int($this->gd_font_file)) { //is a file name
                ...
                if ($font == false) {
                    trigger_error("Failed to load GD Font file {$this->resource_path} {$this->gd_font_file} ", E_USER_WARNING);
                    return;
                }
